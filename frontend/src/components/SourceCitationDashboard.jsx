          ...
          <div key={index} className="glass-panel rounded-xl p-5 hover:border-indigo-500/40 transition-all flex flex-col justify-between space-y-3">
            <div>
              <div className="flex items-start justify-between gap-2 mb-2">
                <span className="text-[10px] font-mono font-semibold uppercase bg-slate-800 text-indigo-300 border border-slate-700 px-2 py-0.5 rounded">
                  Source #{index + 1}
                </span>
                <span className="text-[10px] font-mono bg-emerald-950/80 text-emerald-300 border border-emerald-800 px-2 py-0.5 rounded flex items-center">
                  <CheckCircle className="w-3 h-3 mr-1" /> HTTP {source.status_code || 200}
                </span>
              </div>

              <h4 className="text-sm font-semibold text-slate-100 line-clamp-2 hover:text-indigo-300 transition-colors">
              ...
              {source.sub_query && (
                <div className="mt-2 text-[11px] bg-slate-900/60 text-slate-300 border border-slate-800 px-2.5 py-1 rounded-lg">
                  <span className="text-slate-500 font-mono">Sub-query:</span> {source.sub_query}
                </div>
              )}

              <p className="text-xs text-slate-300 mt-3 line-clamp-4 bg-slate-950/40 p-2.5 rounded-lg border border-slate-800/80 font-serif leading-relaxed">
            ...
            <div className="pt-2 border-t border-slate-800/60 flex items-center justify-between text-[11px] text-slate-400">
              <span className="flex items-center text-slate-400">
                <ShieldCheck className="w-3.5 h-3.5 mr-1 text-emerald-400" /> Fact Verified
              </span>
