...
export default function AuthModal({ isOpen, onClose, onAuthenticate }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm p-4">
      <div className="w-full max-w-md rounded-[32px] border border-slate-800/90 bg-[#06111e] p-6 shadow-2xl">
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={onClose} className="rounded-full p-2 text-slate-400 hover:bg-slate-800/70 hover:text-white transition-all">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="space-y-4">
          <button
            ...
            className={`w-full rounded-3xl px-4 py-3 text-sm font-semibold transition ${mode === 'login' ? 'bg-indigo-600 text-white' : 'bg-slate-900 text-slate-300 hover:bg-slate-800'}`}
          >
            Login with email
          </button>
          <button
            ...
            className={`w-full rounded-3xl px-4 py-3 text-sm font-semibold transition ${mode === 'signup' ? 'bg-indigo-600 text-white' : 'bg-slate-900 text-slate-300 hover:bg-slate-800'}`}
          >
            Create new account
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          ...
          <button
            type="submit"
            className="w-full rounded-3xl bg-indigo-600 px-4 py-3 text-sm font-semibold text-white shadow-lg shadow-indigo-500/20 transition hover:bg-indigo-500"
          >
